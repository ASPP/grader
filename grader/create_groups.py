...
def print_groups(data, K, energy, targets, weights, names):
    ...
    '''
    print 'energy: %.4f' % (energy(data, K, targets, weights))
...
def energy_mudeviation(data, K, mu):
    ...
    '''
    return np.sqrt(np.sum([(np.mean(data[i * K:(i + 1) * K]) - mu) ** 2 for i in xrange(len(data) / K)]))
...
# -> remove weights, weights var with 0.5 compared to mean
# TODO -> adjust ratings in grader.confp
# TODO weights = np.array([2., 2., 2., 2., 1.5])
...
for seed in np.arange(0, Nseeds):
    ...
    data = np.random.permutation(rated)
    initial_E_seeds.append(energy(data, K, targets, weights))
